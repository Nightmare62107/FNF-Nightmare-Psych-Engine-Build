...
local replacementNoteType = 'Blue Bones Note'
...
local blueBonesNoteRandomEnabled = false
...
local function loadToggle()
    ...
    if lines[8] == "bluebonesnoterandom: true" then
        blueBonesNoteRandomEnabled = true
    end
end
...
function onCreatePost()
    loadToggle()
	--debugPrint("Blue Bones enabled = " .. tostring(blueBonesNoteRandomEnabled))
    if not blueBonesNoteRandomEnabled then return end
