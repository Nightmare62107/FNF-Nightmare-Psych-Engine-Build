...
local replacementNoteType = 'Piracy Note'
...
local piracyNoteRandomEnabled = false
...
local function loadToggle()
    ...
    if lines[4] == "piracynoterandom: true" then
        piracyNoteRandomEnabled = true
    end
end
...
function onCreatePost()
    loadToggle()
	--debugPrint("Piracy enabled = " .. tostring(piracyNoteRandomEnabled))
    if not piracyNoteRandomEnabled then return end
