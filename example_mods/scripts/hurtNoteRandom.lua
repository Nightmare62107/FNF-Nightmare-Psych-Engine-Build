...
local replacementNoteType = 'Hurt Note'
...
local hurtNoteRandomEnabled = false
...
local function loadToggle()
    ...
    if lines[1] == "hurtnoterandom: true" then
        hurtNoteRandomEnabled = true
    end
end
...
function onCreatePost()
    loadToggle()
	--debugPrint("Hurt enabled = " .. tostring(hurtNoteRandomEnabled))
    if not hurtNoteRandomEnabled then return end
