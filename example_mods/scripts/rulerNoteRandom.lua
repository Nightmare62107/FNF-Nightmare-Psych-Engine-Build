...
local replacementNoteType = 'Ruler Note'
...
local rulerNoteRandomEnabled = false
...
local function loadToggle()
    ...
    if lines[3] == "rulernoterandom: true" then
        rulerNoteRandomEnabled = true
    end
end
...
function onCreatePost()
    loadToggle()
	--debugPrint("Ruler enabled = " .. tostring(rulerNoteRandomEnabled))
    if not rulerNoteRandomEnabled then return end
