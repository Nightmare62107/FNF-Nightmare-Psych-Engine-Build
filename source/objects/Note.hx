...
class Note extends FlxSprite
{
	...
	public static final defaultNoteTypes:Array<String> = [
		...
		'GF + Player Sing',
		'GF + Opponent Sing',
		'Player + Opponent Sing',
		'Player + Opponent + GF Sing'
	];
	...
	public var gfPlayerNote:Bool = false;
	public var gfOpponentNote:Bool = false;
	public var playerOpponentNote:Bool = false;
	public var playerOpponentGFNote:Bool = false;
	...
	private function set_noteType(value:String):String
	{
		...
		if (noteData > -1 && noteType != value)
		{
			switch(value)
			{
				...
				{
					ignoreNote = mustPress;
					reloadNote('noteSkins/NOTE_assets-ring');
					//this used to change the note texture to HURTNOTE_assets.png,
					//but i've changed it to something more optimized with the implementation of RGBPalette:

					if (PlayState.isPixelStage == true)
					{
						// note colors
				...
				case 'Ruler Note':
				{
					...
					// splash data and colors
					noteSplashData.r = 0xFF420012;
					noteSplashData.g = 0xFFF9A800;
					noteSplashData.texture = 'noteSplashes/noteSplashes-electric';

					// gameplay data
					lowPriority = true;
					missHealth = isSustainNote ? 0.25 : 0.6;
					hitCausesMiss = true;
					//hitsound = 'rulerSlap';
					hitsoundChartEditor = false;
				}
				...
				case 'GF + Opponent Sing':
				{
					gfOpponentNote = true;
				}

				case 'Player + Opponent Sing':
				{
					playerOpponentNote = true;
				}

				case 'Player + Opponent + GF Sing':
				{
					playerOpponentGFNote = true;
				}
			}
	...
	public function reloadNote(texture:String = '', postfix:String = '')
	{
		...
		if(PlayState.isPixelStage)
		{
			if(isSustainNote) {
				...
				var dashIndex = skinPixel.lastIndexOf('-');
				if(dashIndex >= 0) {
					var prefix = skinPixel.substr(0, dashIndex);
					var suffix = skinPixel.substr(dashIndex + 1);
					candidateAlt = 'pixelUI/' + prefix + 'ENDS-' + suffix + skinPostfix;
				}
				var chosen:String = candidateBase;
				if(!Paths.fileExists('images/' + candidateBase + '.png', IMAGE) && candidateAlt.length > 0 && Paths.fileExists('images/' + candidateAlt + '.png', IMAGE)) {
					chosen = candidateAlt;
				}
