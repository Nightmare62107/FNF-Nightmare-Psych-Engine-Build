...
class PsychUINumericStepper extends PsychUIInputText
{
    ...
    public var buttonMultiply:FlxSprite;
    public var buttonDivide:FlxSprite;
    ...
    public function new(x:Float = 0, y:Float = 0, step:Float = 1, defValue:Float = 0, min:Float = -999, max:Float = 999, decimals:Int = 0, ?wid:Int = 60, ?isPercent:Bool = false)
    {
        ...
        buttonPlus = new FlxSprite(fieldWidth, 0).loadGraphic(Paths.image('psych-ui/stepper_plus', 'embed'), true, 16, 16);
        buttonPlus.animation.add('normal', [0], false);
        buttonPlus.animation.add('pressed', [1], false);
        buttonPlus.animation.play('normal');
        add(buttonPlus);
        
        // Minus Button (Far Top-Right)
        buttonMinus = new FlxSprite(fieldWidth + buttonPlus.width, 0).loadGraphic(Paths.image('psych-ui/stepper_minus', 'embed'), true, 16, 16);
        buttonMinus.animation.add('normal', [0], false);
        buttonMinus.animation.add('pressed', [1], false);
        buttonMinus.animation.play('normal');
        add(buttonMinus);

        // Multiply Button (Under Plus)
        buttonMultiply = new FlxSprite(fieldWidth, buttonPlus.height).loadGraphic(Paths.image('psych-ui/stepper_multiply', 'embed'), true, 16, 16);
        buttonMultiply.animation.add('normal', [0], false);
        buttonMultiply.animation.add('pressed', [1], false);
        buttonMultiply.animation.play('normal');
        add(buttonMultiply);

        // Divide Button (Under Minus)
        buttonDivide = new FlxSprite(fieldWidth + buttonPlus.width, buttonMinus.height).loadGraphic(Paths.image('psych-ui/stepper_divide', 'embed'), true, 16, 16);
        buttonDivide.animation.add('normal', [0], false);
        buttonDivide.animation.add('pressed', [1], false);
        buttonDivide.animation.play('normal');
        add(buttonDivide);
    ...
    override function update(elapsed:Float)
    {
        ...
        if(FlxG.mouse.justPressed)
        {
            ...
            else if(buttonMultiply != null && buttonMultiply.exists && FlxG.mouse.overlaps(buttonMultiply, camera))
            {
                buttonMultiply.animation.play('pressed');
                var targetVal:Float = value * 2;
                if(decimals <= 0)
                    value = Math.round(targetVal);
                else
                    value = FlxMath.roundDecimal(targetVal, decimals);
                _internalOnChange();
            }
            else if(buttonDivide != null && buttonDivide.exists && FlxG.mouse.overlaps(buttonDivide, camera))
            {
                buttonDivide.animation.play('pressed');
                var targetVal:Float = value / 2;
                if(decimals <= 0)
                    value = Math.round(targetVal);
                else
        ...
        else if(FlxG.mouse.released)
        {
            ...
            if(buttonMinus != null && buttonMinus.exists && buttonMinus.animation.curAnim != null && buttonMinus.animation.curAnim.name != 'normal')
                buttonMinus.animation.play('normal');
            if(buttonMultiply != null && buttonMultiply.exists && buttonMultiply.animation.curAnim != null && buttonMultiply.animation.curAnim.name != 'normal')
                buttonMultiply.animation.play('normal');
            if(buttonDivide != null && buttonDivide.exists && buttonDivide.animation.curAnim != null && buttonDivide.animation.curAnim.name != 'normal')
                buttonDivide.animation.play('normal');
        }
    }
