...
class PsychUIXBox extends FlxSpriteGroup
{
    public static final CLICK_EVENT = 'xbox_click';
    ...
    {
        box.loadGraphic(Paths.image('psych-ui/xbox', 'embed'), true, 16, 16);
        box.animation.add('false', [0]);
        box.animation.add('true', [1]);
        box.animation.play('false');
    }

    public var broadcastXBoxEvent:Bool = true;
    override function update(elapsed:Float)
    {
        ...
        if(FlxG.mouse.justPressed)
        {
                ...
                (mousePos.y >= screenPos.y && mousePos.y < screenPos.y + height))
            {
                ...
                if(broadcastXBoxEvent) PsychUIEventHandler.event(CLICK_EVENT, this);
            }
        }
    }
