...
class PsychUIDropRightMenu extends PsychUIInputText
{
    public static final CLICK_EVENT = "dropright_click";
    ...
    public function new(x:Float, y:Float, list:Array<String>, callback:Int->String->Void, ?width:Float = 100)
    {
        ...
        button = new FlxSprite(behindText.width + 1, 0).loadGraphic(Paths.image('psych-ui/dropright_button', 'embed'), true, 20, 20);
        ...
        onChange = function(old:String, cur:String)
        {
            if(old != cur)
            {
                ...
                showDropRight(true, 0, _curFilter);
            }
        }
        unfocus = function()
        {
            showDropRightClickFix();
            showDropRight(false);
        }
    ...
    var _items:Array<PsychUIDropRightItem> = [];
    ...
    override function update(elapsed:Float)
    {
        ...
        {
            showDropRight(PsychUIInputText.focusOn == this);
        }
        else if(PsychUIInputText.focusOn == this)
        {
            ...
            if(wheel != 0) showDropRight(true, curScroll + wheel, _curFilter);
        }
    }

    private function showDropRightClickFix()
    {
        if(FlxG.mouse.justPressed)
        {
            for (item in _items)
    ...
    public function showDropRight(vis:Bool = true, scroll:Int = 0, onlyAllowed:Array<String> = null)
    {
        if(!vis)
        {
            text = selectedLabel;
        ...
        if(vis)
        {
            ...
            // Drop RIGHT: stack items to the right of the input box
            var txtX:Float = behindText.x + behindText.width + 1;
            ...
            bg.scale.x = txtX - behindText.x - behindText.width + 2;
            bg.x = behindText.x + behindText.width;
            bg.updateHitbox();
        }
        else
        {
    ...
    public var broadcastDropRightEvent:Bool = true;
    function clickedOn(num:Int, label:String)
    {
        selectedIndex = num;
        showDropRight(false);
        if(onSelect != null) onSelect(num, label);
        if(broadcastDropRightEvent) PsychUIEventHandler.event(CLICK_EVENT, this);
    }
    ...
    function addOption(option:String)
    {
        ...
        var item:PsychUIDropRightItem = cast recycle(PsychUIDropRightItem, () -> new PsychUIDropRightItem(1, 1, this._itemWidth), true);
...
class PsychUIDropRightItem extends FlxSpriteGroup
{
