...
class PsychUIDropUpMenu extends PsychUIInputText
{
	public static final CLICK_EVENT = "dropup_click";
	...
	public function new(x:Float, y:Float, list:Array<String>, callback:Int->String->Void, ?width:Float = 100)
	{
		...
		button = new FlxSprite(behindText.width + 1, 0).loadGraphic(Paths.image('psych-ui/dropup_button', 'embed'), true, 20, 20);
	...
	var _items:Array<PsychUIDropUpItem> = [];
	...
	override function update(elapsed:Float)
	{
		...
		{
			showDropUp(PsychUIInputText.focusOn == this);
		}
		else if(PsychUIInputText.focusOn == this)
		{
	...
	private function showDropUpClickFix()
	{
		if(FlxG.mouse.justPressed)
		{
			for (item in _items) //extra update to fix a little bug where it wouldnt click on any option if another input text was behind the drop up
				if(item != null && item.active && item.visible)
					item.update(0);
		}
	}

	public function showDropUp(vis:Bool = true, scroll:Int = 0, onlyAllowed:Array<String> = null)
	{
		if(!vis)
		{
			text = selectedLabel;
		...
		if(vis)
		{
			...
			// Drop UP: stack items above the input box
			var txtY:Float = behindText.y - 1;
			...
			bg.scale.y = behindText.y - txtY + 2;
			bg.y = txtY;
			bg.updateHitbox();
		}
		else
	...
	public var broadcastDropUpEvent:Bool = true;
	function clickedOn(num:Int, label:String)
	{
		selectedIndex = num;
		showDropUp(false);
		if(onSelect != null) onSelect(num, label);
		if(broadcastDropUpEvent) PsychUIEventHandler.event(CLICK_EVENT, this);
	}
	...
	function addOption(option:String)
	{
		...
		var item:PsychUIDropUpItem = cast recycle(PsychUIDropUpItem, () -> new PsychUIDropUpItem(1, 1, this._itemWidth), true);
...
class PsychUIDropUpItem extends FlxSpriteGroup
{
